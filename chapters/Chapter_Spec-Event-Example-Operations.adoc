...
Runtime Operation Events are "occurrences" that are happening during the running of application on a specific platform or system resource (server, data base, broker, etc). 
For system resources, they are focused mostly on management of the resource. 
For application programs, the events are related to debugging and application logic tracing.

=== Platform Processing Event  ===
These events related to the action of completing a discreet process or unit of work and providing the resource computation of that unit of work. 
...
The event can be used for both operational and application events where counting, and resource utilization reporting is of interest to the business.
...
These events are related to the actions of managing a system resource manager.  This includes the overall operations of the resource manager and the system administrator actions to administer the resource manager.
