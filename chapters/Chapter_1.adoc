...
Before the adoption of serverless computing models, often it was required to have provision servers for maximum requests, that may sit idle, one can have fewer workers in the background continuously managing the work. However, in a serverless model, while the number of workers will scale based on incoming load, there is still a concern about cold-start times for each worker. An event driven architecture following an asynchronous approach is typically a more efficient use of computing resources than a synchronous approach.
...
* Sophisticated applications are leveraging more domain (or managed) services that are being provided by parties who are outside of the control of the organization and the central application. 
This leads to more interactions and increases user experience wait time when the services are executed in a sequential manner.
