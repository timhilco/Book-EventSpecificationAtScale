...
=== Overview ===
The purpose of this chapter is to define the overall design specification for events. 
...
This is more for efficient use of the networking infrastructure than domain processing of the event.

.Supports schema version control and message validation
Event are not stagnant.
They will continually evolve, which makes version control a first class element within the message.
...
This information helps to route the request to the proper stream route and to facilitate proper processing of the event by a component within the stream topology.
...
The two headers contain the metadata about the event, the _context_ of the event. T
he body contains the actual data related to the event. 
==== Common Message Header ====

The Common Message Header provides the following key features:

.Message Metadata
The Message Metadata contains key information about the message, a unique, global message identifier, the type of message, creation timestamp, original publisher and history of consumer processors.
...
.Event Context
The event context are fields related to when the event was published.
...
The event context provides fields for the retrieval of the main subject business entities and any additional related resources involved at the time of publication.
...
Since this is JSON, routing or filtering consumers can use only the header to determine routing of message or if the consumer is interested in processing the message. This provides a high degree of standardization, which leads to excellent tooling.
...
* To determine the <eventTypeheader> name, the header.messageDataSchema field contains the name of the event type
* To determine the body schema name, the header.eventBodyDataSchema field determine the name for the body schema
...
subjectIdentifier:: Describes the global identity of the subject being acted upon. The 'subject' is typically a key business domain object.
